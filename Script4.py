...
# with an inital lookback period of 2500 days, a confidence of 99% for 5 days into the future. For the Monte Carlo
# method, 2000 simulations were chosen. This script compares the three values and profiles the code.
...
# Read and clean the multi asset data with a lookback of 5000 business days. Read separately for historical data.
...
histdirtydata = readData.alternativeHistReturns(["CHRIS/ICE_CC4.1", "CHRIS/ASX_UB6.1", "CHRIS/ASX_WM1.1", "CHRIS/ASX_VW1.1", "CHRIS/ASX_WM2.1"], 2500, 5)
cleanHistData = readData.cleanData(dirtydata)
...
paraVaR = parametricVaR.paraPortfolioVaR(cleandata, [0.1, 0.2, 0.3, 0.25, 0.15], 99, 5)
...
timesteplist = list(range(6))
monteReturns = monteCarloVaR.portfolioMonteCarlo2(cleandata, timesteplist, [0.1, 0.2, 0.3, 0.25, 0.15], 2000)
